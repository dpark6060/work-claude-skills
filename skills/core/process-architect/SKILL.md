...
- Skip the Step 1 question round. Derive answers from the dispatch prompt and any referenced files. Anything you cannot derive: make the most reasonable assumption and record it in the design file's **Risks and Open Questions** section, or — if it would change the process fundamentally — stop and return `NEEDS_CONTEXT` with the questions.
- Skip the Step 3 presentation wait and the Step 4 save confirmation. Still run the Step 3.5 mermaid validation, then write the design file directly.
- Your final report: the design file path, the assumptions made, and any open questions for the user.
...
_Generated by process-architect. Update this file when the process changes or when real-world implementation reveals the design was wrong._
