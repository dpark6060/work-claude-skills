...
> Place in
> "~/.claude/plugins/cache/claude-plugins-official/skill-creator/<id>>/skills/skill-creator/references
> And add one line to the "references" section of the SKILL.md:
> - `references/community-insights.md` — Community-discovered patterns: trigger rate research, caveman token compression, learnings.md self-improvement, and real-world rules of thumb. Read when the user asks about token efficiency, undertriggering, or community best practices.
>
...
**When to suggest to users:** Any skill that will be run many times and where the failure
modes aren't fully known upfront. Particularly good for data-processing skills, API
integration skills, and any skill interacting with external systems.
